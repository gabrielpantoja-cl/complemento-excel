...
**Pasos realizados:**
1. Iniciar sesion en https://entra.microsoft.com con Gmail
2. Crear tenant (se creo automaticamente al registrar)
3. El tenant queda con dominio GUID (sin nombre personalizado)

**Pendiente:**
- [ ] Crear usuario de trabajo `peritajes@<dominio>.onmicrosoft.com`
- [ ] Usar ese usuario para entrar a Partner Center
- [ ] Seleccionar rol ISV en Partner Center
- [ ] Completar registro de la organizacion

### 11.3 Publicacion del complemento

Una vez con cuenta Partner Center activa:

1. Crear nueva oferta en Partner Center → **Office Add-in**
2. Subir `manifest.prod.xml` (ya generado, apunta a Vercel)
3. Configurar listing:
   - Nombre: "Tasaciones by Loxos"
   - Descripcion en español
   - Capturas de pantalla del complemento funcionando
   - Iconos (ya tenemos icon-16, 32, 80, 128)
   - Categoria: Productivity / Data & Analytics
4. Configurar URLs:
   - Sitio web: `https://tasaciones-for-excel.vercel.app`
   - Soporte: GitHub Issues
   - Politica de privacidad (necesaria)
   - Terminos de uso (necesarios)
5. Enviar para revision de Microsoft
6. Esperar aprobacion (tipicamente 3-7 dias habiles)

### 11.4 Notas sobre dominios

El dominio de Vercel (`tasaciones-for-excel.vercel.app`) funciona para
desarrollo y sideload. Para la version final de AppSource se evaluara:

- `tasaciones.gabrielpantoja.cl` — subdominio del dominio personal
- `tasaciones.loxos.cl` — subdominio corporativo de Loxos
- `tasaciones-for-excel.vercel.app` — mantener como fallback

El cambio de dominio se hace actualizando `ADDIN_BASE_URL` y regenerando
el manifest.prod.
