...
void test("PRESET_PROVIDERS exposes every required field", () => {
  for (const preset of PRESET_PROVIDERS) {
    ...
    assert.ok(preset.kind === "openai-completions", `${preset.id}: only openai-completions is supported today`);
  }
});
...
void test("buildPresetProviderRecord stores a CustomProvider-shaped record", () => {
